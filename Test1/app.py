...
def get_doc_owner_name(number):
    doc_exist = check_document_existance(number)
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
...
def append_doc_to_shelf(doc_number, shelf_number):
    ...
    return doc_number in directories[shelf_number]
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            user_doc_number = input('Введите номер документаa - ')
